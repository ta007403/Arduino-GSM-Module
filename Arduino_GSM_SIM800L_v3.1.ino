...
#define GSM_POWER_RELAY_ACTIVE_HIGH
...
void setup()
{
  ...
  #ifdef GSM_POWER_RELAY_ACTIVE_HIGH
    digitalWrite(GSM_POWER_RELAY_PIN, LOW); // Turn off for active high relay
  #else
    digitalWrite(GSM_POWER_RELAY_PIN, HIGH); // Turn off for active low relay
  #endif
...
void resetGSMModule() {
  ...
  #ifdef GSM_POWER_RELAY_ACTIVE_HIGH
    digitalWrite(GSM_POWER_RELAY_PIN, HIGH);  // Turn on for active high relay
  #else
    digitalWrite(GSM_POWER_RELAY_PIN, LOW);  // Turn on for active low relay
  #endif
    
  digitalWrite(LED_PIN, HIGH);
  digitalWrite(LED_BUILTIN, HIGH);
  delay(20000);                              // Keep off for 20 sec
